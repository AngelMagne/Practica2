...
public class EstudianteController { // Define la clase EstudianteController
    ...
    @GetMapping("/{id}") // Anotación que indica que este método maneja solicitudes GET con un parámetro de ruta
    public ResponseEntity<EstudianteDTO> obtenerEstudiantePorId(@PathVariable Long id) { // Método para obtener un estudiante por su ID
        ...
        EstudianteDTO estudiante = estudianteService.obtenerEstudiantePorId(id); // Llama al servicio para obtener el estudiante
        long fin = System.currentTimeMillis();
        logger.info("[ESTUDIANTE] Fin obtenerEstudiantePorId: {} (Duracion: {} ms)", fin, (fin-inicio));
