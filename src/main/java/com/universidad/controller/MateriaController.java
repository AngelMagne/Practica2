...
public class MateriaController {
    ...
    @Transactional
    @PutMapping("/{idMateria}/asignar-docente/{idDocente}")
    public ResponseEntity<?> asignarDocente(
            @PathVariable Long idMateria,
            @PathVariable Long idDocente) {
        try {
            // Obtener la materia y el docente
            Materia materia = materiaService.obtenerEntidadMateriaPorId(idMateria); // Debes tener este método en tu servicio
            Docente docente = docenteService.obtenerEntidadDocentePorId(idDocente);

            // Asignar el docente a la materia
            materia.setDocente(docente);
            materiaService.guardarEntidad(materia); // Debes tener este método en tu servicio

            return ResponseEntity.ok("Docente asignado correctamente a la materia.");
        } catch (Exception e) {
